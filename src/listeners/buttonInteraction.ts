    ...
    switch (interaction.customId) {
      ...
      case 'music_queue': {
        ...
        for (let i = 0; i < tracks.length; i += tracksPerPage) {
          ...
          const description = page.map((t, idx) => {
          ...
          const embed = new EmbedBuilder()
            ...
            .setFooter({ text: `Page ${Math.floor(i / tracksPerPage) + 1}/${Math.ceil(tracks.length / tracksPerPage)} • ${tracks.length} total tracks • Loop: ${queue.loop}` });
      ...
      if (playerTotal > 21) {
        ...
        const embed = new EmbedBuilder()
          ...
          .addFields([
            ...
            { name: '🎴 Dealer', value: `${formatHand(game.dealerHand)}\nTotal: **${calculateHand(game.dealerHand)}**` }
          ]);
        ...
        const embed = new EmbedBuilder()
          ...
          .addFields([
            { name: '🎴 Your Hand', value: `${formatHand(game.playerHand)}\nTotal: **${playerTotal}**` },
            { name: '🎴 Dealer', value: `${game.dealerHand[0]!.value}${game.dealerHand[0]!.suit} 🂠` }
          ]);
