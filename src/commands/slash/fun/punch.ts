...
export const punch = {
  ...
  async execute(client: Client, interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    try {
      ...
      const url = `https://tenor.googleapis.com/v2/search?q=${encodeURIComponent('anime punch')}&key=${apiKey}&client_key=zenitsubot&limit=50&media_filter=gif&contentfilter=high`;
      ...
      const description = !target || target.id === interaction.user.id 
        ? `**${interaction.user.username}** punches the air! 👊`
        : `**${interaction.user.username}** punches **${target.username}**! 👊`;
      ...
      logger.error({ err }, 'Punch command error');
