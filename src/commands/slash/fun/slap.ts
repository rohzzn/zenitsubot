...
export const slap = {
  ...
  async execute(client: Client, interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    try {
      ...
      const url = `https://tenor.googleapis.com/v2/search?q=${encodeURIComponent('anime slap')}&key=${apiKey}&client_key=zenitsubot&limit=50&media_filter=gif&contentfilter=high`;
      ...
      const description = !target || target.id === interaction.user.id 
        ? `**${interaction.user.username}** slaps themselves! 😤`
      ...
      logger.error({ err }, 'Slap command error');
