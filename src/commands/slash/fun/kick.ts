...
export const kickfun = {
  data: { name: 'kickfun', description: 'Kick someone (fun, not moderation)!' },
  async execute(client: Client, interaction: ChatInputCommandInteraction): Promise<void> {
    const target = interaction.options.getUser('user');
    await interaction.deferReply();
    try {
      const apiKey = 'AIzaSyAyimkuYQYF_FXVALexPuGQctUWRURdCYQ';
      const url = `https://tenor.googleapis.com/v2/search?q=${encodeURIComponent('anime kick')}&key=${apiKey}&client_key=zenitsubot&limit=50&media_filter=gif&contentfilter=high`;
      ...
      const description = !target || target.id === interaction.user.id 
        ? `**${interaction.user.username}** kicks the air! 🦵`
        : `**${interaction.user.username}** kicks **${target.username}**! 🦵`;
      ...
      logger.error({ err }, 'Kick fun command error');
