...
export const animecharacter = {
  ...
  async execute(client: Client, interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    try {
      ...
      const detailsResponse = await fetch(`https://api.jikan.moe/v4/characters/${character.mal_id}/full`);
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields([
          { name: '❤️ Favorites', value: fullChar.favorites?.toLocaleString() || '0', inline: true },
      ...
      if (data.data.length > 1) {
        otherResults = '\n**Other Results:**\n' + data.data.slice(1, 4).map((c: any, i: number) => 
          `${i + 2}. [${c.name}](${c.url}) - ${c.favorites?.toLocaleString() || 0} ❤️`
        ).join('\n');
      }
