  ...
  async execute(client: Client, interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    try {
      ...
      const reviewsResponse = await fetch(
        `https://api.jikan.moe/v4/anime/${anime.mal_id}/reviews?limit=3`,
      );
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields([
          {
            name: 'Score',
            value: `${fullAnime.score || 'N/A'}/10 (${fullAnime.scored_by?.toLocaleString() || 0} users)`,
            inline: true,
          },
          { name: 'Rank', value: `#${fullAnime.rank || 'N/A'}`, inline: true },
          { name: 'Popularity', value: `#${fullAnime.popularity || 'N/A'}`, inline: true },
          { name: 'Episodes', value: fullAnime.episodes?.toString() || 'Unknown', inline: true },
          { name: '⏱ Duration', value: fullAnime.duration || 'Unknown', inline: true },
          ...
          {
            name: 'Genres',
            value: fullAnime.genres?.map((g: any) => g.name).join(',') || 'N/A',
            inline: true,
          },
        ])
      ...
      embed.setFooter({
        text: `MyAnimeList • Members: ${fullAnime.members?.toLocaleString() || 'N/A'}`,
      });
      embed.setTimestamp();
