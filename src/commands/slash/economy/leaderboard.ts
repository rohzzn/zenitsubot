...
export const leaderboard = {
  ...
  async execute(client: Client, interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    if (topUsers.length === 0) {
      await interaction.editReply({ content: `${EMOTES.CONFUSED_CAT} No economy data yet! Start using commands! 💛` });
      return;
    }
    ...
    const description = await Promise.all(
      topUsers.map(async (user, index) => {
        try {
          ...
          const medal = index === 0 ? '🥇' : index === 1 ? '🥈' : index === 2 ? '🥉' : `\`${(index + 1).toString().padStart(2, '0')}.\``;
    ...
    const embed = new EmbedBuilder()
      ...
      .setTitle(`${EMOTES.FLUENT_SPARKLES} Global ${type === 'coins' ? 'Wealth' : 'Level'} Leaderboard`)
      .setDescription(
